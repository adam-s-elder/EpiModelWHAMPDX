...
make_epi_plot_data <- function(dat_obj, plot_params) {
  ...
  if (is.data.frame(dat_obj$epi[[1]]$num)) {
     sim.idx <- 1:ncol(dat_obj$epi[[1]]$num)
     dat_obj$epi <- map(sim.idx, function(x) {
       lapply(dat_obj$epi[[1]], function(y){y[, x]})
     })
  }else{
    sim.idx <- 1
  }
  ...
  for (plt_idx in seq_along(expnd_plot_params)) {
    ...
    if (!is.null(sub_params$vars)) {
      upd_epi <- lapply(dat_obj$epi, EpiModelWHAMPDX::make_new_epi,
                        sub_params$vars, sub_params$sum_fun,
                        paste0(sub_params$name, "."))
    }else{
      upd_epi <- dat_obj$epi
    }
    all.inf <- lapply(sim.idx, function(x) {
      req_epi <- list()
      ...
      if (ncol(all_epi_inf) - sum(which_all_na) < 2) {
        stop("There are too many missing values for your constructed variable",
             ". All values appear to be missing. ",
             "Make sure you are not dividing by zero or NA.")
      }
      all_epi_inf <- all_epi_inf[, !which_all_na, drop = FALSE]
      ...
      tidy_epi_info$simno <- x
      return(tidy_epi_info)
    })
    tidy_epi_info <- do.call(bind_rows, all.inf)
    if (mult_meas) {
